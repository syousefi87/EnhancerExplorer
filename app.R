...
FB.enhancer_Vista <- read.fst( "FB_Enhancer/FB_Vista.table.bed",from = 1,to = NULL,as.data.table = T,old_format = FALSE)
FB_Gene <- read.fst( "FB_Enhancer/FB_hg19.Gene.bed",from = 1,to = NULL,as.data.table = T,old_format = FALSE)
FB_OMIM <- read.fst( "FB_Enhancer/FB_phenotype.hg19.OMIM.bed",from = 1,to = NULL,as.data.table = T,old_format = FALSE)
FB_Clinvar <- read.fst( "FB_Enhancer/FB_phenotype.hg19.ClinVar.bed",from = 1,to = NULL,as.data.table = T,old_format = FALSE)
FB_GWAS <- read.fst( "FB_Enhancer/FB_phenotype.hg19.GWAS.bed",from = 1,to = NULL,as.data.table = T,old_format = FALSE)
FB_DNA.features.table <- read.fst( "FB_Enhancer/FB_GC.phastCons.nCER.LoF.bed",from = 1,to = NULL,as.data.table = T,old_format = FALSE)
FB.Expression.melt <- read.fst( "FB_Enhancer/Expression_melt.bed",from = 1,to = NULL,as.data.table = T,old_format = FALSE)
...
server <- function (input, output, session) {
  ...
  FB_Expression_table <- reactive({
    data.frame(fread('FB_Enhancer/Expression_table.bed')) %>% 
      subset (GeneName %in% c(toupper(FB.Expression.gene.query()))) %>% unique()
  })
